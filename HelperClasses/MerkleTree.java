...
public class MerkleTree {
  ...
  public List<Pair<String, String>> SiblingPath(int t_id)
  {
    int h = numdocs;
    int l = 0;
    int v = (l+h)/2;
    int d = (int)(Math.log(numdocs)/Math.log(2));
    List<Pair<String, String>> A = new ArrayList<Pair<String,String>>(1+d);
    TreeNode n = rootnode;
    Pair<String, String> P = new Pair<String, String>(n.val, null);
    A.add(P);
    for(int i=0; i<d; i++)
    {
      Pair<String, String> p = new Pair<String, String>(n.left.val, n.right.val);
      A.add(p);
      if(t_id<v)
      {
        n = n.left;
